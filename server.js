...
const sequelize = new Sequelize('egg_tap', 'egg_user', 'eggenjoyer', {
  host: 'localhost',
  dialect: 'postgres'
});
...
const bot = new Telegraf('7481598644:AAEP2ts2PDU6CI7pMnnCkXE1PhxbEoqH7ss');
// WebApp URL
const webAppUrl = 'https://egg-tap.ru';
  ...
  GET: {
    '/get-referral-link': async (req, res, query) => {
      ...
      try {
        ...
        if (user) {
          const inviteLink = `https://t.me/egg_tap_bot?start=${user.referralCode}`;
...
const options = {
    key: fs.readFileSync('/etc/letsencrypt/live/egg-tap.ru/privkey.pem'),
    cert: fs.readFileSync('/etc/letsencrypt/live/egg-tap.ru/fullchain.pem')
};
...
const server = https.createServer(options, async (req, res) => {
  ...
  if (routes[method] && routes[method][pathname]) {
    ...
    let filePath = path.join(__dirname, '..', 'egg', req.url === '/' ? 'main.html' : req.url);
...
server.listen(httpsPort, () => {
  ...
  console.log(`HTTPS Сервер запущен на https://egg-tap.ru`);
});
